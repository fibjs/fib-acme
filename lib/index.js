...
function AcmeApp(opts) {
    ...
    const renew_cert = () => {
        if (cfg.cert.cert) {
            var cert = new crypto.X509Certificate(cfg.cert.cert);
            if (new Date(cert.validTo) - Date.now() > renewBefore * 24 * 60 * 60 * 1000)
                return;
        }
        ...
        order = {
            ...
            csr: base64.encode(crypto.createCertificateRequest({
                key: cfg.cert.key,
                subject: {
                    "CN": cfg.domain
                }
    ...
    if (!cfg.key)
        cfg.key = crypto.generateKeyPair('ec', {
            namedCurve: crv
        }).privateKey;
    else {
        console.log(cfg);
        cfg.key = crypto.createPrivateKey(cfg);
    }
    ...
    if (!cfg.kid) {
        ...
        var res = _request({
            ...
            jwk: crypto.createPublicKey(cfg.key).export({
                format: "jwk"
            })
        }, {
    ...
    if (!cfg.cert.key)
        cfg.cert.key = crypto.generateKeyPair('ec', {
            namedCurve: crv
        }).privateKey;
    else
        cfg.cert.key = crypto.createPrivateKey({
            key: cfg.cert.key
        });
